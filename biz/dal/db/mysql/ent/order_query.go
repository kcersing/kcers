...
// QueryAmount chains the current query on the "amount" edge.
func (oq *OrderQuery) QueryAmount() *OrderAmountQuery {
	query := (&OrderAmountClient{config: oq.config}).Query()
...
func (oq *OrderQuery) QueryItem() *OrderItemQuery {
...
func (oq *OrderQuery) QueryPay() *OrderPayQuery {
	query := (&OrderPayClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrderQuery) QuerySales() *OrderSalesQuery {
	query := (&OrderSalesClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryOrderVenues chains the current query on the "order_venues" edge.
func (oq *OrderQuery) QueryOrderVenues() *VenueQuery {
	query := (&VenueClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryOrderMembers chains the current query on the "order_members" edge.
func (oq *OrderQuery) QueryOrderMembers() *MemberQuery {
	query := (&MemberClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryOrderCreates chains the current query on the "order_creates" edge.
func (oq *OrderQuery) QueryOrderCreates() *UserQuery {
...
// WithAmount tells the query-builder to eager-load the nodes that are connected to
// the "amount" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithAmount(opts ...func(*OrderAmountQuery)) *OrderQuery {
	query := (&OrderAmountClient{config: oq.config}).Query()
...
func (oq *OrderQuery) WithItem(opts ...func(*OrderItemQuery)) *OrderQuery {
	query := (&OrderItemClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrderQuery) WithSales(opts ...func(*OrderSalesQuery)) *OrderQuery {
	query := (&OrderSalesClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithOrderVenues tells the query-builder to eager-load the nodes that are connected to
// the "order_venues" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithOrderVenues(opts ...func(*VenueQuery)) *OrderQuery {
	query := (&VenueClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withOrderVenues = query
	return oq
}

// WithOrderMembers tells the query-builder to eager-load the nodes that are connected to
// the "order_members" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithOrderMembers(opts ...func(*MemberQuery)) *OrderQuery {
	query := (&MemberClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withOrderMembers = query
	return oq
}

// WithOrderCreates tells the query-builder to eager-load the nodes that are connected to
// the "order_creates" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithOrderCreates(opts ...func(*UserQuery)) *OrderQuery {
	query := (&UserClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withOrderCreates = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withAmount; query != nil {
		if err := oq.loadAmount(ctx, query, nodes,
			func(n *Order) { n.Edges.Amount = []*OrderAmount{} },
			func(n *Order, e *OrderAmount) { n.Edges.Amount = append(n.Edges.Amount, e) }); err != nil {
			return nil, err
		}
	}
	if query := oq.withItem; query != nil {
		if err := oq.loadItem(ctx, query, nodes,
			func(n *Order) { n.Edges.Item = []*OrderItem{} },
			func(n *Order, e *OrderItem) { n.Edges.Item = append(n.Edges.Item, e) }); err != nil {
			return nil, err
		}
	}
	if query := oq.withPay; query != nil {
		if err := oq.loadPay(ctx, query, nodes,
	...
	if query := oq.withSales; query != nil {
	...
	if query := oq.withOrderVenues; query != nil {
		if err := oq.loadOrderVenues(ctx, query, nodes, nil,
			func(n *Order, e *Venue) { n.Edges.OrderVenues = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withOrderMembers; query != nil {
		if err := oq.loadOrderMembers(ctx, query, nodes, nil,
			func(n *Order, e *Member) { n.Edges.OrderMembers = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withOrderCreates; query != nil {
		if err := oq.loadOrderCreates(ctx, query, nodes, nil,
			func(n *Order, e *User) { n.Edges.OrderCreates = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadAmount(ctx context.Context, query *OrderAmountQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderAmount)) error {
...
func (oq *OrderQuery) loadItem(ctx context.Context, query *OrderItemQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderItem)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (oq *OrderQuery) loadPay(ctx context.Context, query *OrderPayQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderPay)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (oq *OrderQuery) loadSales(ctx context.Context, query *OrderSalesQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderSales)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (oq *OrderQuery) loadOrderVenues(ctx context.Context, query *VenueQuery, nodes []*Order, init func(*Order), assign func(*Order, *Venue)) error {
...
func (oq *OrderQuery) loadOrderMembers(ctx context.Context, query *MemberQuery, nodes []*Order, init func(*Order), assign func(*Order, *Member)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Order)
	for i := range nodes {
		fk := nodes[i].MemberID
...
func (oq *OrderQuery) loadOrderCreates(ctx context.Context, query *UserQuery, nodes []*Order, init func(*Order), assign func(*Order, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Order)
	for i := range nodes {
