...
type VenueQuery struct {
	...
	withVenueEntry           *VenueEntryQuery
...
func (vq *VenueQuery) QueryVenueEntry() *VenueEntryQuery {
	query := (&VenueEntryClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryMemberPropertyVenues chains the current query on the "member_property_venues" edge.
func (vq *VenueQuery) QueryMemberPropertyVenues() *MemberProductPropertyQuery {
	query := (&MemberProductPropertyClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryPropertyVenues chains the current query on the "property_venues" edge.
func (vq *VenueQuery) QueryPropertyVenues() *ProductPropertyQuery {
	query := (&ProductPropertyClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VenueQuery) WithVenueEntry(opts ...func(*VenueEntryQuery)) *VenueQuery {
	query := (&VenueEntryClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithMemberPropertyVenues tells the query-builder to eager-load the nodes that are connected to
// the "member_property_venues" edge. The optional arguments are used to configure the query builder of the edge.
func (vq *VenueQuery) WithMemberPropertyVenues(opts ...func(*MemberProductPropertyQuery)) *VenueQuery {
	query := (&MemberProductPropertyClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withMemberPropertyVenues = query
	return vq
}

// WithPropertyVenues tells the query-builder to eager-load the nodes that are connected to
// the "property_venues" edge. The optional arguments are used to configure the query builder of the edge.
func (vq *VenueQuery) WithPropertyVenues(opts ...func(*ProductPropertyQuery)) *VenueQuery {
	query := (&ProductPropertyClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VenueQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Venue, error) {
	...
	if query := vq.withVenueEntry; query != nil {
		if err := vq.loadVenueEntry(ctx, query, nodes,
			func(n *Venue) { n.Edges.VenueEntry = []*VenueEntry{} },
			func(n *Venue, e *VenueEntry) { n.Edges.VenueEntry = append(n.Edges.VenueEntry, e) }); err != nil {
			return nil, err
		}
	}
	if query := vq.withMemberPropertyVenues; query != nil {
		if err := vq.loadMemberPropertyVenues(ctx, query, nodes,
			func(n *Venue) { n.Edges.MemberPropertyVenues = []*MemberProductProperty{} },
			func(n *Venue, e *MemberProductProperty) {
				n.Edges.MemberPropertyVenues = append(n.Edges.MemberPropertyVenues, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := vq.withPropertyVenues; query != nil {
...
func (vq *VenueQuery) loadVenueOrders(ctx context.Context, query *OrderQuery, nodes []*Venue, init func(*Venue), assign func(*Venue, *Order)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(entorder.FieldVenueID)
	}
...
func (vq *VenueQuery) loadVenueEntry(ctx context.Context, query *VenueEntryQuery, nodes []*Venue, init func(*Venue), assign func(*Venue, *VenueEntry)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Venue)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(venueentry.FieldVenueID)
	}
...
func (vq *VenueQuery) loadMemberPropertyVenues(ctx context.Context, query *MemberProductPropertyQuery, nodes []*Venue, init func(*Venue), assign func(*Venue, *MemberProductProperty)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(venue.MemberPropertyVenuesTable)
		s.Join(joinT).On(s.C(memberproductproperty.FieldID), joinT.C(venue.MemberPropertyVenuesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(venue.MemberPropertyVenuesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(venue.MemberPropertyVenuesPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*MemberProductProperty](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "member_property_venues" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (vq *VenueQuery) loadPropertyVenues(ctx context.Context, query *ProductPropertyQuery, nodes []*Venue, init func(*Venue), assign func(*Venue, *ProductProperty)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Venue)
	nids := make(map[int64]map[*Venue]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(venue.PropertyVenuesTable)
		s.Join(joinT).On(s.C(productproperty.FieldID), joinT.C(venue.PropertyVenuesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(venue.PropertyVenuesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(venue.PropertyVenuesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "property_venues" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
