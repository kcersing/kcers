...
func (rq *RoleQuery) QueryVenues() *VenueQuery {
	query := (&VenueClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withVenues: rq.withVenues.Clone(),
...
func (rq *RoleQuery) WithVenues(opts ...func(*VenueQuery)) *RoleQuery {
	query := (&VenueClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withVenues; query != nil {
		if err := rq.loadVenues(ctx, query, nodes,
			func(n *Role) { n.Edges.Venues = []*Venue{} },
			func(n *Role, e *Venue) { n.Edges.Venues = append(n.Edges.Venues, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadVenues(ctx context.Context, query *VenueQuery, nodes []*Role, init func(*Role), assign func(*Role, *Venue)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Role)
	nids := make(map[int64]map[*Role]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		...
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.VenuesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
