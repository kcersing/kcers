...
func (*Member) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case member.FieldPassword, member.FieldUsername, member.FieldMobile, member.FieldName, member.FieldAvatar:
...
func (m *Member) QueryMemberProfile() *MemberProfileQuery {
	return NewMemberClient(m.config).QueryMemberProfile(m)
}
