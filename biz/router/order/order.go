...
func Register(r *server.Hertz) {
	...
	root := r.Group("/", rootMw()...)
	{
		_service := root.Group("/service", _serviceMw()...)
		{
			...
			_order.POST("/all-count", append(_orderallcountMw(), order.OrderAllCount)...)
			_order.POST("/buy", append(_buyMw(), order.Buy)...)
			_order.POST("/contract-sign-member", append(_contractsignmemberMw(), order.ContractSignMember)...)
